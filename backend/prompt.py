...
- 「新人」/「新員工」等詞彙需對照 RAG 中的角色對照表，對每位新進員工各產生一筆記錄
- 「老員工」/「正職」等詞彙同上，需展開成個別員工 ID
- 所有陣列若無對應約束請輸出空陣列 []
- 不要輸出 JSON 以外的任何文字
'''
...
def _build_rag_context(employees: List[Employee], daily_staff_count: Optional[int]) -> str:
    ...
    # Explicitly map role keywords so LLM can resolve "新人" → concrete employee IDs
    ...
    lines.append("→ 遇到「新人」類詞彙時，必須對每位新進員工各產生一筆約束記錄。")
